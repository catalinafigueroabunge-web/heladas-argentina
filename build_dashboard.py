...
function buildStepMarkers(data, key, stepFn, tipFn){{
  ...
  data.forEach(d=>{{
    ...
    const sz=3.5+Math.min(1,d[key]/FROST_BP[FROST_BP.length-1].thr)*7.5;
    L.circleMarker([d.lat,d.lon],{{
      radius:sz,color:'white',weight:.7,
...
function buildMarkersFrom(data, key, colorFn, tipFn){{
  ...
  data.forEach(d=>{{
    ...
    L.circleMarker([d.lat,d.lon],{{
      radius:3.5+t*7.5,color:'white',weight:.7,
...
function buildDivergingMarkers(data, key, tipFn){{
  ...
  data.forEach(d=>{{
    ...
    const sz=3.5+Math.min(1,Math.abs(d[key])/absMax)*7.5;
    L.circleMarker([d.lat,d.lon],{{
      radius:sz,color:'white',weight:.7,
